-- Recovery installer for a failed Actually Useful Turtles host installation
-- Run ONLY on the new mining controller; do not use on GPS computers or turtles.
-- This refreshes project files and resets the controller's default config.
...
-- Fix existing runtime copies as well as source files. Keep runtime saved state.
fs.makeDir("runtime")
for _, folder in ipairs({"general", "gui", "host", "storage"}) do
  for _, filename in ipairs(fs.list(folder)) do
    local source = folder .. "/" .. filename
    if not fs.isDir(source) then
      local destination = "runtime/" .. filename
...
assert(fs.getSize("runtime/initialize.lua") > 100, "Initializer was not copied correctly")
print("Repair complete. Type: reboot")
